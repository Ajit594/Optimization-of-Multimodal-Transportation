...
## Project Overview
There are numerous modes of conveyance accessible for delivery services, including trucks, aircraft, and ships. Costs will vary depending on the routes and transportation tools selected. We should take into account various transportation expenses, delivery time constraints, and products consolidation—occurring when diverse goods travel together—in order to optimize costs. This project models such a scenario and finds an overall cost-minimization solution through mathematical programming. Two mathematical programming frameworks are available for the model construction: **DOcplex** and **CVXPY**.

IBM developed **DOcplex**, a commercial programming framework that makes direct calls to the **CPLEX** solver. It is necessary to install **CPLEX Studio** before you can use this framework option. Comprehensive information about the Stanford research team's original development of the open-source **CVXPY** programming framework may be found [**here**].the Cvxpy at https://github.com/cvxgrp. This option allows you to select from a list of solvers integrated by the **CVXPY** framework; the **CBC** solver (open-source, requires installation) is suggested for this model; further information is available [**here**].Choose a solver (https://www.cvxpy.org/tutorial/advanced/index.html#).
...
## Problem Statement
There are eight items, four cities/countries (Shanghai, Wuxi, Singapore, Malaysia), sixteen ports, and four transportation tools in our fictitious scenario. The eight items have distinct destinations and come from various cities. There are four ports in every city or nation: the seaport, railroad station, airport, and warehouse. There are fifty direct routes linking various ports. Every route has a weekly timetable, a specific transportation tool, a transportation cost, and a transit time. Every city has a warehouse where products can be placed for a set amount of time to accommodate specific transportation timetables or to wait to be transported with other items. There may be variations in the order dates and delivery deadlines for every commodity. How can we determine the paths of solution for all items that minimize the overall cost? 
...
Finally, hope this long documentation helps, and thanks all my Bytes teammates (**Ajit, Shivam, Titiksha**) for the help and contributions in this project!
